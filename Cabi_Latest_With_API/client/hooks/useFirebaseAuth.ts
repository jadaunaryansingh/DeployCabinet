...
const createUserFromFirebase = (firebaseUser: FirebaseUser, userType: 'user' | 'developer'): User => {
  // Determine the proper level based on email
  let level = 'Standard Member';
  let name = firebaseUser.displayName || firebaseUser.email?.split('@')[0] || 'User';
  
  if (userType === 'developer') {
    if (firebaseUser.email === 'aryan@cabinet.com') {
      level = 'CTO';
      name = 'Aryan Singh';
    } else if (firebaseUser.email === 'utkarsh@cabinet.com') {
      level = 'CEO';
      name = 'Utkarsh Yadav';
    } else if (firebaseUser.email === 'anant@cabinet.com') {
      level = 'COO';
      name = 'Anant Goyal';
    } else {
      level = 'Developer Admin';
    }
  }
  ...
  return {
    ...
    name: name,
    avatar: firebaseUser.photoURL || `https://ui-avatars.com/api/?name=${encodeURIComponent(name)}&background=d89000&color=000000&size=200`,
    level: level,
    rating: 5.0,
    totalRides: 0,
    joinedDate: new Date(),
...
const mapFirestoreToUser = (firebaseUser: FirebaseUser, userData: any): User => {
  ...
  // Determine proper name and level based on email for developers
  let name = userData.name || firebaseUser.displayName || '';
  let level = userData.level || 'Standard Member';
  
  // Override with proper developer titles based on email
  if (firebaseUser.email === 'aryan@cabinet.com') {
    name = 'Aryan Singh';
    level = 'CTO';
  } else if (firebaseUser.email === 'utkarsh@cabinet.com') {
...
export function useAuthProvider() {
  ...
  const login = async (email: string, password: string, userType: 'user' | 'developer' = 'user'): Promise<boolean> => {
    ...
    try {
      ...
      if (savedUser) {
        try {
          ...
          if (parsedUser.email === email || parsedUser.firebaseUid === firebaseUser.uid) {
            console.log('Loading user from localStorage, but updating userType to:', userType);
            // Update the userType and level to match the current login attempt
            let updatedLevel = parsedUser.level;
            if (userType === 'developer') {
              if (email === 'aryan@cabinet.com') updatedLevel = 'CTO';
              else if (email === 'utkarsh@cabinet.com') updatedLevel = 'CEO';
              else if (email === 'anant@cabinet.com') updatedLevel = 'COO';
              else updatedLevel = 'Developer Admin';
            }
            user = { ...parsedUser, userType, level: updatedLevel };
          } else {
      ...
      console.log('User logged in successfully:', user.name, 'UserType:', user.userType);
      return true;
    } catch (error) {
      console.error('Login error:', error);
      
      // Firebase authentication failed - NO FALLBACK
  ...
  useEffect(() => {
    ...
    if (!auth) {
      // Firebase not configured - NO DEMO FALLBACK
      setAuthState({
        user: null,
        isLoggedIn: false,
        isLoading: false
